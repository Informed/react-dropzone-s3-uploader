...
var DropzoneS3Uploader = (function (_React$Component) {
  ...
  function DropzoneS3Uploader(props) {
    ...
    this.handleFinish = function (info, file) {
      ...
      var uploaders = _this.state.uploaders;

      var newUploaders = Object.assign({}, uploaders);
      delete newUploaders[file.name];
      _this.setState({ uploadedFiles: uploadedFiles, uploaders: newUploaders, error: null, progress: null }, function () {
    ...
    this.handleDrop = function (files, rejectedFiles) {
      ...
      var newUploaders = Object.assign({}, uploaders);
      var uploader = new _reactS3UploaderS3upload2['default'](options); // eslint-disable-line
      files.forEach(function (file) {
        newUploaders[file.name] = uploader;
      });
      _this.setState({ uploaders: newUploaders });
      _this.props.onDrop && _this.props.onDrop(files, rejectedFiles);
    };

    this.abort = function (filename) {
      var uploaders = _this.state.uploaders;

      uploaders[filename] && uploaders[filename].abortUpload();
      delete uploaders[filename];
      _this.setState({ uploaders: uploaders });
    };
    ...
    var uploaders = {};
    this.state = { uploadedFiles: uploadedFiles, uploaders: uploaders };
  }
